...
| AC-UI-3 | Top row: search input + reset × + chevron `>` anchor button, no border-bottom divider | Screenshot: drawer open empty state | Search input on left with placeholder "Search products…"; × visible on right end of input only when query non-empty; chevron-right close button on right edge of drawer, vertically centered with the input row; no horizontal divider line below the row | | | |
...
| AC-UI-5 | Chevron `>` anchor closes drawer | Interactive: click chevron-right on drawer right edge → screenshot | Drawer closes (slides out to the right) | | | |
