...
| AC-UI-13 | Top Categories multi-select reuses Menu Builder's `CheckboxListContent` (search + Added/Available sections) | Interactive: open multi-select, pick 3 categories → screenshot showing search input + "Added" section (3 checked items) + "Available" section (unchecked items) | Visually consistent with `AddCategoriesDropdown` in Menu Builder; checking moves item from Available to Added; unchecking does the reverse | | | |
| AC-UI-14 | Top Categories: 6-cap UX | Interactive: select 6 categories, attempt 7th → screenshot | When 6 are checked, all items in "Available" section render disabled; "6 / 6 selected" hint visible above the list; clicking a disabled item does nothing | | | |
