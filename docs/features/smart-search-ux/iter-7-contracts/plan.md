...
**File:** `lib/ai/voice-surfaces.server.ts` + DB schema (if needed)

On load: hash the current generation prompt string (`crypto.createHash("sha256").update(promptString).digest("hex")`). Compare to `store.voiceSurfacePromptHash`. If different → regen + store new hash.
...
### Commit 6: Neutralize extraction prompt examples
...
| `lib/ai/extraction.ts` | 1, 2, 6 | Schema-derived prompt spec, merch keywords instruction, example neutralization |
| `app/api/search/route.ts` | 2, 3 | Merch query uses productKeywords; compare/recommend shortcircuit |
| `lib/ai/voice-surfaces.server.ts` | 4 | Prompt hash check + regen on mismatch |
