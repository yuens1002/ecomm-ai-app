...
1. Create `help-content.ts` with view descriptions (bulleted text)
2. Create `HelpPopoverButton.tsx`
3. Update `MenuActionBar/index.tsx` to render help button

### Phase 3: Delete Button

1. Create `DeleteAlertButton.tsx`
2. Update `builder-state.ts` types (add `deleteSelected`)
3. Implement `deleteSelected` action with full undo/redo
4. Update `MenuActionBar/index.tsx` to render delete button

### Phase 4: Keyboard Shortcuts

1. Create `useKeyboardShortcuts.ts` hook
2. Integrate into MenuBuilder provider
3. Test all shortcuts

### Phase 5: Testing & Polish

1. Test help popovers on all views
2. Test delete with AlertDialog + undo/redo on all-labels and all-categories
3. Test all keyboard shortcuts
4. Verify focus context handling (inputs, modals)
