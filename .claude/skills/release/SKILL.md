...
- **Vercel trigger**: Vercel reads the version from `package.json` at build time. Tags alone don't trigger a deployment — the `package.json` bump commit (Phase A) is what triggers the build.
...
- **Companion repo:** [`artisan-roast-platform`](../../../../artisan-roast-platform/.claude/skills/release/SKILL.md) has its own generic `/release` skill (no GitHub Release step, no npm-script delegation). The two repos diverge intentionally because their cadences differ — the platform repo has no customer upgrade path.
