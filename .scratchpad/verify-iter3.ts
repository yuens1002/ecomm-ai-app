...
const OUTPUT_DIR = path.join(process.cwd(), ".screenshots", "verify-iter3");
...
async function main() {
  ...
  for (const bp of BREAKPOINTS) {
    ...
    await new Promise(r => setTimeout(r, 3000));
    await page.screenshot({
      path: path.join(OUTPUT_DIR, `orders-${bp.name}.png`),
      fullPage: true,
    });
    await page.close();
  }
  ...
  for (const bp of BREAKPOINTS) {
    ...
    // Also click the dots menu on a subscription card to verify AC-UI-32
    if (bp.name === "mobile") {
      const dotsBtn = await page.$('[class*="space-y-2"] button[class*="ghost"]');
      if (dotsBtn) {
        await dotsBtn.click();
        await new Promise(r => setTimeout(r, 800));
        await page.screenshot({
          path: path.join(OUTPUT_DIR, `subscriptions-dropdown-${bp.name}.png`),
          fullPage: false,
        });
      }
    }
